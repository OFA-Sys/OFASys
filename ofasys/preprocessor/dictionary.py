...
class Dictionary:
    ...
    def get_start_end_idx(self, prefix: str):
        start = -1
        end = -2
        for i, token in enumerate(self.symbols):
    ...
    def add_symbol(self, word, n=1, overwrite=False, check=True):
        """Adds a word to the dictionary"""
        if check and word not in self.indices:
            prefix = word.split('_', 1)[0]
            if prefix in self.all_prefixes and self.symbols[-1].split('_', 1)[0] != prefix:
                logger.warning(f"Adding symbol '{word}' failed!!")
                return -1
            else:
    ...
    def add_from_file(self, f, prefix=None, check=True):
        """
        ...
        if isinstance(f, str):
            try:
                with open(PathManager.get_local_path(f), "r", encoding="utf-8") as fd:
                    self.add_from_file(fd, prefix=prefix)
        ...
        for line in lines[indices_start_line:]:
            try:
                ...
                if word in self and not overwrite:
                    ...
                    # )
                    # TODO: check here. we do not need add duplicate word
                    pass
                if prefix:
                    word = prefix + '_' + word
                self.add_symbol(word, n=count, overwrite=overwrite, check=check)
            except ValueError:
                raise ValueError("Incorrect dictionary format, expected " f"'<token> <cnt> [flags]': \"{line}\"")
