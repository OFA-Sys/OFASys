...
from ofasys.module.hifigan import Generator as HiFiGANModel
from ofasys.utils.audio_feature_transforms.data_cfg import S2TDataConfig
from ofasys.utils.audio_utils import (
    TTSSpectrogram,
    get_fourier_basis,
    get_mel_filters,
    get_window,
...
class GriffinLim(torch.nn.Module):
    ...
    def inverse(self, magnitude: torch.Tensor, phase) -> torch.Tensor:
        ...
        win_sum_sq = self.get_window_sum_square(
            magnitude.shape[-1], hop_length=self.hop_length, win_length=self.win_length, n_fft=self.n_fft
        ).to(magnitude.device)
...
class GriffinLimVocoder(nn.Module):
    def __init__(
        self, sample_rate, win_size, hop_size, n_fft, n_mels, f_min, f_max, window_fn, spec_bwd_max_iter=32, fp16=False
    ):
        super().__init__()
        self.inv_mel_transform = PseudoInverseMelScale(
            n_stft=n_fft // 2 + 1, n_mels=n_mels, sample_rate=sample_rate, f_min=f_min, f_max=f_max
        )
        self.gl_transform = GriffinLim(
            n_fft=n_fft, win_length=win_size, hop_length=hop_size, window_fn=window_fn, n_iter=spec_bwd_max_iter
        )
        if fp16:
            self.half()
...
class HiFiGANVocoder(nn.Module):
    def __init__(self, checkpoint_path: str, model_cfg: Dict[str, str], fp16: bool = False) -> None:
        super().__init__()
