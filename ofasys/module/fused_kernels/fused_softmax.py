...
def attention_mask_func(attn_mask):
    if attn_mask is not None:
        attn_mask = attn_mask.masked_fill(attn_mask.to(torch.bool), -1e8 if attn_mask.dtype == torch.float32 else -1e4)
    return attn_mask
...
class FusedScaleMaskSoftmax(nn.Module):
    ...
    def __init__(
        ...
        self.scaled_masked_softmax_fusion = attention_mask_func
    ...
    def forward(self, input, mask):
        # [b, np, sq, sk]
        assert input.dim() == 4, input.shape
    ...
    def is_kernel_available(self, input, mask, b, np, sq, sk):
        ...
        if (
            ...
            and input.is_cuda  # input is in cuda
