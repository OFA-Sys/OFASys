    ...
    def _apply(x):
        if torch.is_tensor(x):
            ...
            for field in x.__dataclass_fields__:
                setattr(x, field, _apply(getattr(x, field)))
            return x
        else:
            return x
...
def move_to_cuda(sample, device=None):
    ...
    cuda_sample = apply_to_sample(_move_to_cuda, sample)
...
def load_align_dict(replace_unk):
    if replace_unk is None:
        ...
        # Load alignment dictionary for unknown word replacement if it was passed as
        # an argument.
        align_dict = {}
...
def replace_unk(hypo_str, src_str, alignment, align_dict, unk):
    from ofasys.preprocessor import tokenizer
    ...
    for i, ht in enumerate(hypo_tokens):
        if ht == unk:
            ...
            # Either take the corresponding value in the aligned dictionary or
...
def get_perplexity(loss, round=2):
...
def get_activation_fn(activation: str) -> Callable:
    ...
    from ofasys.module import gelu, gelu_accurate
...
def new_arange(x, *size):
    ...
    if len(size) == 0:
        size = x.size()
    return torch.arange(size[-1], device=x.device).expand(*size)
