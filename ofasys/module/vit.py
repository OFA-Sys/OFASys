...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, drop_path_rate=0.0):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.resblocks = nn.Sequential(
            *[ResidualAttentionBlock(width, heads, attn_mask, drop_path_rate) for _ in range(layers)]
        )
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if resolution != self.input_resolution:
            ...
            old_pe = old_pe.reshape(1, patch_num, patch_num, -1).permute(0, 3, 1, 2)
            new_pe = F.interpolate(old_pe, size=(height, width), mode="bilinear")
