...
from ofasys.configure import BaseDataclass, ChoiceEnum
...
@dataclass
class EncDecBaseConfig(BaseDataclass):
...
@dataclass
class QuantNoiseConfig(BaseDataclass):
    pq: float = field(
        default=0.0,
...
@dataclass
class TransformerConfig(BaseDataclass):
    ...
    def from_namespace(cls, args):
        ...
        if not isinstance(args, cls):
            ...
            args_dict = (
                args._asdict() if hasattr(args, '_asdict') else vars(args) if hasattr(args, '__dict__') else {}
