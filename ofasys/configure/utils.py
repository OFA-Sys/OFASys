...
def gen_parser_from_dataclass(
    ...
    dataclass_instance: BaseDataclass,
    ...
    def get_kwargs_from_dc(dataclass_instance: BaseDataclass, k: str) -> Dict[str, Any]:
...
def _override_attr(sub_node: str, data_class: Type[BaseDataclass], args: Namespace) -> List[str]:
    overrides = []

    if not inspect.isclass(data_class) or not issubclass(data_class, BaseDataclass):
        return overrides
    ...
    for k, v in data_class.__dataclass_fields__.items():
        ...
        elif isinstance(val, BaseDataclass):
...
def override_module_args(args: Namespace) -> Tuple[List[str], List[str]]:
    ...
    for k in TrainerConfig.__dataclass_fields__.keys():
        overrides.extend(_override_attr(k, TrainerConfig.__dataclass_fields__[k].type, args))
    ...
    if args is not None:
        ...
        from ofasys.configure.registry import REGISTRIES
...
def convert_namespace_to_omegaconf(args: Namespace) -> DictConfig:
    ...
    with initialize(version_base=None, config_path=config_path):
        try:
            composed_cfg = compose("default_trainer", overrides=overrides)
        except:
...
def merge_with_parent(dc: BaseDataclass, cfg: DictConfig, remove_missing=True):
    if remove_missing:
