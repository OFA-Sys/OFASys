...
from .utils import gen_parser_from_dataclass
...
def parse_args_and_arch(
    ...
    # Extract OFA-related args to config store
    ConfigStore().import_args(args)
...
def get_parser(desc, default_task="translation"):
    ...
    # add ofasys-related arguments to the parser, first ofasys configuration group
    # is added and parsed, then only the args related to the groups are added
    ConfigStore().add_args(parser)

    from ofasys.configure.registry import REGISTRIES

    for registry_name, REGISTRY in REGISTRIES.items():
        if registry_name == 'criterion':
            continue
        parser.add_argument(
