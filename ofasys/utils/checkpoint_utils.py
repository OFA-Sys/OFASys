...
from ofasys.configure.configs import CheckpointConfig
from ofasys.configure.utils import (
    convert_namespace_to_omegaconf,
    overwrite_args_by_name,
)
from ofasys.distributed.fully_sharded_data_parallel import FSDP, has_FSDP
...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itrs, val_loss, step=0):
    if cfg.no_save or not cfg.save_dir:
        return

    from ofasys.logging import meters
    ...
    epoch_itr = list(epoch_itrs.values())[0]
    ...
    if len(epoch_itrs) == 1:
        ...
        logger.info(f"Preparing to save checkpoint for {updates} updates")
    ...
    checkpoint_conds["checkpoint_{}{}.pt".format(updates, suffix)] = (
    ...
    iterator_state = {}
    for task_name, epoch_itr in epoch_itrs.items():
        iterator_state[task_name] = epoch_itr.state_dict()

    extra_state = {
        "train_iterator": iterator_state,
        "val_loss": val_loss,
        "global_step": step + 1,
    }
    ...
    if len(checkpoints) > 0:
        ...
        logger.info(
            ...
            "(writing took {} seconds)".format(checkpoints[0], epoch, updates, val_loss, write_timer.sum)
        )
    ...
    *passthrough_args* will be passed through to
    ...
    if cfg.restore_file == "checkpoint_last.pt":  # default value of restore_file is 'checkpoint_last.pt'
        ...
        if cfg.finetune_from_model is not None and first_launch:
            # if there is no last checkpoint to restore,
            # start the finetune from pretrained model
    ...
    if extra_state is not None and not reset_dataloader:
        ...
        for task in trainer.tasks:
            itr_state[task.name] = None
    trainer.init_train_iterator(itr_state)
...
def load_checkpoint_to_cpu(path, arg_overrides=None, load_on_all_ranks=False):
    ...
    if "cfg" in state and state["cfg"] is not None:
        ...
        except omegaconf.errors.UnsupportedValueType:
            # TODO: fix omegaconf convert namespace error in checkpoint.cfg
            pass
    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            if shard_idx % 10 == 0 and shard_idx > 0:
                ...
                logger.info(f"Loaded {shard_idx} shards in {elapsed:.2f}s, " f"{elapsed / (shard_idx+1):.2f}s/shard")
...
def load_pretrained_component_from_model(component: Union["BaseEncoder", "BaseDecoder"], checkpoint: str):
    """
    ...
    if isinstance(component, BaseEncoder):
        component_type = "encoder"
    elif isinstance(component, BaseDecoder):
        component_type = "decoder"
    else:
        raise ValueError(
            "component to load must be either a BaseEncoder or "
            "BaseDecoder. Loading other component types are not supported."
        )
