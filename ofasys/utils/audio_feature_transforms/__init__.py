...
def register_audio_feature_transform(name):
    def register_audio_feature_transform_cls(cls):
        ...
        if not issubclass(cls, AudioFeatureTransform):
            raise ValueError(f"Transform ({name}: {cls.__name__}) must extend " "AudioFeatureTransform")
        if cls.__name__ in AUDIO_FEATURE_TRANSFORM_CLASS_NAMES:
            raise ValueError(f"Cannot register audio feature transform with duplicate " f"class name ({cls.__name__})")
...
for file in os.listdir(transforms_dir):
    ...
    if not file.startswith("_") and not file.startswith(".") and (file.endswith(".py") or os.path.isdir(path)):
        ...
        importlib.import_module("ofasys.utils.audio_feature_transforms." + name)
...
class CompositeAudioFeatureTransform(AudioFeatureTransform):
    ...
    def from_config_dict(cls, config=None):
        ...
        if _transforms is None:
            return None
        transforms = [get_audio_feature_transform(_t).from_config_dict(_config.get(_t)) for _t in _transforms]
