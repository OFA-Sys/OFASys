...
class SpecAugmentTransform(AudioFeatureTransform):
    ...
    def __init__(
        ...
        if freq_mask_n > 0:
            assert freq_mask_f > 0, f"freq_mask_F ({freq_mask_f}) " f"must be larger than 0 when doing freq masking."
        if time_mask_n > 0:
            assert time_mask_t > 0, f"time_mask_T ({time_mask_t}) must be larger than 0 when " f"doing time masking."
