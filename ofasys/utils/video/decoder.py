...
def get_multiple_start_end_idx(
    video_size, clip_sizes, clip_idx, num_clips_uniform, min_delta=0, max_delta=math.inf, use_offset=False
):
    """
    Sample a clip of size clip_size from a video of size video_size and
    return the indices of the first and last frame of the clip. If clip_idx is
...
def torchvision_decode(
    ...
    for i in range(len(sampling_rate)):
        if sampling_rate[i] < 0:
            sampling_rate[i] = math.floor(fps * video_meta["video_duration"] / abs(sampling_rate[i]) / num_frames[i])

    for i in range(len(sampling_rate)):
        sampling_rate[i] = int(round(sampling_rate[i]))
    ...
    return sampling_rate, frames_out, fps, decode_all_video, start_end_delta_time
...
def pyav_decode(
    ...
    for i in range(len(sampling_rate)):
        if sampling_rate[i] < 0:
            sampling_rate[i] = math.floor(frames_length / abs(sampling_rate[i]) / num_frames[i])
...
def decode(
    ...
    try:
        if backend == "pyav":
            ...
            sampling_rate, frames_decoded, fps, decode_all_video, start_end_delta_time = pyav_decode(
                container,
                sampling_rate,
                num_frames,
                clip_idx,
            ...
            (sampling_rate, frames_decoded, fps, decode_all_video, start_end_delta_time,) = torchvision_decode(
                container,
                sampling_rate,
                num_frames,
                clip_idx,
