    ...
    if spatial_idx == -1:
        if aspect_ratio is None and scale is None:
            if use_spatial_sampling:
                ...
                frames, _ = transform.random_both_side_scale_jitter(
                    images=frames,
                    min_size=min_scale,
                    max_size=max_scale,
                    inverse_uniform_sampling=inverse_uniform_sampling,
            ...
            transform_func = (
                transform.random_resized_crop_with_shift if motion_shift else transform.random_resized_crop
            )
        ...
        if use_spatial_sampling:
            ...
            frames, _ = transform.random_both_side_scale_jitter(frames, min_scale, max_scale)
    return frames
