...
def random_both_side_scale_jitter(images, min_size, max_size, boxes=None, inverse_uniform_sampling=False):
    # TODO: Handle bounding boxes scaling.
    assert boxes is None, "Scaling bounding boxes not supported yet."
    if inverse_uniform_sampling:
        size = int(round(1.0 / np.random.uniform(1.0 / max_size, 1.0 / min_size)))
    else:
        size = int(round(np.random.uniform(min_size, max_size)))
