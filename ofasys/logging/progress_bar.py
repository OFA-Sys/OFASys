...
from ofasys.io.reader import dataset
...
def progress_bar(
    ...
    if log_format == "json":
        bar = JsonProgressBar(iterator, start, epoch, prefix, log_interval)
    elif log_format == "none":
        bar = NoopProgressBar(iterator, start, epoch, prefix)
    elif log_format == "simple":
        bar = SimpleProgressBar(iterator, start, epoch, prefix, log_interval)
    elif log_format == "tqdm":
        bar = TqdmProgressBar(iterator, start, epoch, prefix)
    else:
...
class BaseProgressBar(object):
    ...
    def __init__(self, iterable, start=0, epoch=None, prefix=None):
        self.iterable = iterable
        self.n = getattr(iterable, "n", start)

        if isinstance(self.iterable, dataset.CountingIterator):
            self.active_iterable = self.iterable.skip(self.n)
        else:
            self.active_iterable = self.iterable[self.n :]
...
class JsonProgressBar(BaseProgressBar):
    ...
    def __init__(self, iterable, start=0, epoch=None, prefix=None, log_interval=1000):
        super().__init__(iterable, start, epoch, prefix)
    ...
    def __iter__(self):
        ...
        for i, obj in enumerate(self.active_iterable, start=self.n):
            self.i = i
            yield obj
...
class NoopProgressBar(BaseProgressBar):
    ...
    def __iter__(self):
        for obj in self.active_iterable:
            yield obj
