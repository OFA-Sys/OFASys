...
@dataclass
class PolynomialDecayLRScheduleConfig(BaseDataclass):
...
@register_lr_scheduler("ofa_polynomial_decay", dataclass=PolynomialDecayLRScheduleConfig)
class PolynomialDecayLRSchedule(FairseqLRScheduler):
    ...
    def reinit(self, total_num_update, num_updates):
        ...
        self.warmup_updates = max(int(self.total_num_update * self.cfg.warmup_ratio), 1)
        if num_updates > 0:
