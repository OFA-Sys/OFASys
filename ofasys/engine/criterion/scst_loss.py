...
@dataclass
class ScstRewardCriterionConfig(CriterionConfig):
...
@register_config("ofasys.criterion", "scst_reward_criterion", ScstRewardCriterionConfig)
class ScstRewardCriterion(BaseCriterion):
    """This criterion computes Self-critical Sequence Training (SCST) loss, which is proposed for image captioning.
    ...
    def __init__(self, task, cfg: CriterionConfig):
        ...
        local_path = cached_path(cfg.scst_cider_cached_tokens)
        self.scst_cider_scorer = CiderD(df=local_path)
    ...
    def forward(self, model, sample, update_num=0, reduce=True):
        ...
        loss, score, ntokens, nsentences = self.compute_loss(model, sample_copy, reduce=reduce)
    ...
    def get_generator_out(self, model, sample):
        ...
        for i in range(len(gen_out)):
            ...
            gt_res.append(decode(utils.strip_pad(sample["target"][i], self.padding_idx))[1].split('&&'))
    ...
    def get_net_output(self, model, sample, gen_target):
        ...
        for slot in sample['net_input']['slots']:
            if slot.is_src:
                slot.value = torch.repeat_interleave(slot.value, seq_per_img, dim=0)
            else:
                assert slot.modality.value == 1
                slot.value = torch.as_tensor(
                    merge(gen_target, eos=self.task.tgt_dict.bos(), move_eos_to_beginning=True),
                    device=sample["target"].device,
                    dtype=torch.int64,
                )

        gen_target_tokens = torch.as_tensor(merge(gen_target), device=sample["target"].device, dtype=torch.int64)
    ...
    def reduce_metrics(cls, logging_outputs, prefix_name=None) -> None:
        ...
        metrics.log_scalar(f"{task_name}score", score_sum / nsentences, nsentences, round=3)

        metrics.log_scalar(f"{task_name}ntokens", ntokens, 1, round=3)
        metrics.log_scalar(f"{task_name}nsentences", nsentences, 1, round=3)
