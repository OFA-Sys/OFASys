...
from ofasys.module.diffusion import build_denoise_fn


@register_config("ofasys.criterion", "diffusion_criterion", CriterionConfig)
class DiffusionCriterion(BaseCriterion):
    def __init__(self, task, cfg: CriterionConfig):
        super().__init__(task, cfg)
        self.diffusion = task.diffusion
    ...
    def forward(self, model, sample, update_num=0, reduce=True):
        ...
        loss, x_predict = self.diffusion.p_losses(denoise_fn=denoise_fn, x_start=x_start)  # [B,T,D]
        ...
        if hasattr(slot_preproc, 'custom_reg_loss'):
            aux_loss_1 = slot_preproc.custom_reg_loss(slot, x_predict, x_start)
        ...
        if hasattr(slot_adaptor, 'custom_reg_loss'):
            aux_loss_2 = slot_adaptor.custom_reg_loss(slot, x_predict, x_start)
    ...
    def reduce_metrics(cls, logging_outputs, prefix_name=None) -> None:
        ...
        for k in ["loss", "main_loss", "aux_loss_1", "aux_loss_2"]:
            ...
            metrics.log_scalar(f"{task_name}{k}", loss_sum / sample_size, sample_size, round=3)
            if k == "loss":
                metrics.log_scalar(f"loss", loss_sum / sample_size, sample_size, priority=0, round=3)
